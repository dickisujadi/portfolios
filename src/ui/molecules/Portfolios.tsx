                ...
                <div className="w-3/12 sm:hidden">
                    {experiences && experiences.length && experiences.map((_experience, idx) => (
                        <img 
                            ...
                            className={`cursor-pointer hover:mix-blend-normal ${_experience.id === experience.id ? 'mix-blend-normal' : 'mix-blend-luminosity'} my-5vh`} 
...
>
  {experience.company}
  <p className="hidden sm:block rotate-90 ml-4 ml-auto">{`>`}</p>
</div>
                    ...
                    <div className="my-5vh">
                        ...
                        <ul className="list-disc">
                            {experience.jobDesc && experience.jobDesc.length && experience.jobDesc.map((jobDesc, idx) => (
                                <li className="text-lg my-5 sm:text-base" key={`${experience.id}-${idx}`}>{jobDesc}</li>
                            ))}
                        </ul>
                    </div>
                </div>
