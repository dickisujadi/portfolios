...
export default function Header({ content, onClick }: HeaderProps) {
    ...
    return <>
        <div className={`fixed w-screen min-h-[10vh] ${isMenuAppears&& 'min-h-[15vh]'} ${isScrolled && 'shadow-xl'} bg-white z-20 flex flex-row items-center justify-center xs:flex-col`}>
            <div className={`xs:block hidden ${isMenuAppears&& 'w-screen px-[30vw]'}`} onClick={setMenuHandler}>
                {isMenuAppears? <GrFormClose /> : <TbMenu />}
            </div>
